"use strict";function _typeof(obj){"@babel/helpers - typeof";if(typeof Symbol==="function"&&typeof Symbol.iterator==="symbol"){_typeof=function _typeof(obj){return typeof obj;};}else{_typeof=function _typeof(obj){return obj&&typeof Symbol==="function"&&obj.constructor===Symbol&&obj!==Symbol.prototype?"symbol":typeof obj;};}return _typeof(obj);}document.addEventListener("DOMContentLoaded",function(){/*!
...
;Toast._jQueryInterface=function _jQueryInterface(config){return this.each(function(){var $element=$__default['default'](this);var data=$element.data(DATA_KEY$a);var _config=_typeof(config)==='object'&&config;if(!data){data=new Toast(this,_config);$element.data(DATA_KEY$a,data);}if(typeof config==='string'){if(typeof data[config]==='undefined'){throw new TypeError("No method named \""+config+"\"");}data[config](this);}});};_createClass(Toast,null,[{key:"VERSION",get:function get(){return VERSION$a;}},{key:"DefaultType",get:function get(){return DefaultType$7;}},{key:"Default",get:function get(){return Default$7;}}]);return Toast;}();/**
   ...
   * jQuery
   * ------------------------------------------------------------------------
   */$__default['default'].fn[NAME$a]=Toast._jQueryInterface;$__default['default'].fn[NAME$a].Constructor=Toast;$__default['default'].fn[NAME$a].noConflict=function(){$__default['default'].fn[NAME$a]=JQUERY_NO_CONFLICT$a;return Toast._jQueryInterface;};exports.Alert=Alert;exports.Button=Button;exports.Carousel=Carousel;exports.Collapse=Collapse;exports.Dropdown=Dropdown;exports.Modal=Modal;exports.Popover=Popover;exports.Scrollspy=ScrollSpy;exports.Tab=Tab;exports.Toast=Toast;exports.Tooltip=Tooltip;exports.Util=Util;Object.defineProperty(exports,'__esModule',{value:true});});;(function($,undefined){'use strict';var defaults={item:3,autoWidth:false,slideMove:1,slideMargin:10,addClass:'',mode:'slide',useCSS:true,cssEasing:'ease',//'cubic-bezier(0.25, 0, 0.25, 1)',
...
index:false,iframeMaxWidth:'100%',download:true,counter:true,appendCounterTo:'.lg-toolbar',swipeThreshold:50,enableSwipe:true,enableDrag:true,dynamic:false,dynamicEl:[],galleryId:1,supportLegacyBrowser:true};function Plugin(element,options){// Current lightGallery element
...
if(_this.s.hideBarsDelay>0){// Hide controllers if mouse doesn't move for some period
...
if(this.s.controls&&this.$items.length>1){controls='<div class="lg-actions">'+'<button type="button" aria-label="Previous slide" class="lg-prev lg-icon">'+this.s.prevHtml+'</button>'+'<button type="button" aria-label="Next slide" class="lg-next lg-icon">'+this.s.nextHtml+'</button>'+'</div>';}if(this.s.appendSubHtmlTo==='.lg-sub-html'){subHtmlCont='<div role="status" aria-live="polite" class="lg-sub-html"></div>';}var ariaLabelledby=this.s.ariaLabelledby?'aria-labelledby="'+this.s.ariaLabelledby+'"':'';var ariaDescribedby=this.s.ariaDescribedby?'aria-describedby="'+this.s.ariaDescribedby+'"':'';template='<div tabindex="-1" aria-modal="true" '+ariaLabelledby+' '+ariaDescribedby+' role="dialog" class="lg-outer '+this.s.addClass+' '+this.s.startClass+'">'+'<div class="lg" style="width:'+this.s.width+'; height:'+this.s.height+'">'+'<div class="lg-inner">'+list+'</div>'+'<div class="lg-toolbar lg-group">'+'<button type="button" aria-label="Close gallery" class="lg-close lg-icon"></button>'+'</div>'+controls+subHtmlCont+'</div>'+'</div>';$('body').append(template);this.$outer=$('.lg-outer');this.$outer.focus();this.$slide=this.$outer.find('.lg-item');if(this.s.useLeft){this.$outer.addClass('lg-use-left');// Set mode lg-slide if use left is true;
...
this.s.speed=0;}this.$outer.addClass(this.s.mode);if(this.s.enableDrag&&this.$items.length>1){this.$outer.addClass('lg-grab');}if(this.s.showAfterLoad){this.$outer.addClass('lg-show-after-load');}if(this.doCss()){var $inner=this.$outer.find('.lg-inner');$inner.css('transition-timing-function',this.s.cssEasing);$inner.css('transition-duration',this.s.speed+'ms');}setTimeout(function(){$('.lg-backdrop').addClass('in');});setTimeout(function(){_this.$outer.addClass('lg-visible');},this.s.backdropDuration);if(this.s.download){this.$outer.find('.lg-toolbar').append('<a id="lg-download" aria-label="Download" target="_blank" download class="lg-download lg-icon"></a>');}// Store the current scroll top value to scroll back after closing the gallery..
...
var support=function support(){var transition=['transition','MozTransition','WebkitTransition','OTransition','msTransition','KhtmlTransition'];var root=document.documentElement;var i=0;for(i=0;i<transition.length;i++){if(transition[i]in root.style){return true;}}};if(support()){return true;}return false;};/**
     ...
     *  Ex: 1/10
     */Plugin.prototype.counter=function(){if(this.s.counter){$(this.s.appendCounterTo).append('<div id="lg-counter" role="status" aria-live="polite"><span id="lg-counter-current">'+(parseInt(this.index,10)+1)+'</span> / <span id="lg-counter-all">'+this.$items.length+'</span></div>');}};/**
...
touchNext=0;touchPrev=_length-1;}}else{touchPrev=0;touchNext=1;}if(direction==='prev'){_this.$slide.eq(touchNext).addClass('lg-next-slide');}else{_this.$slide.eq(touchPrev).addClass('lg-prev-slide');}_this.$slide.eq(index).addClass('lg-current');}if(_this.lGalleryOn){setTimeout(function(){_this.loadContent(index,true,0);},this.s.speed+50);setTimeout(function(){_this.lgBusy=false;_this.$el.trigger('onAfterSlide.lg',[_prevIndex,index,fromTouch,fromThumb]);},this.s.speed);}else{_this.loadContent(index,true,_this.s.backdropDuration);_this.lgBusy=false;_this.$el.trigger('onAfterSlide.lg',[_prevIndex,index,fromTouch,fromThumb]);}_this.lGalleryOn=true;if(this.s.counter){$('#lg-counter-current').text(index+1);}}_this.index=index;};/**
...
_this.$outer.on('mousedown.lg',function(e){if($(e.target).is('.lg-outer')||$(e.target).is('.lg-item ')||$(e.target).is('.lg-img-wrap')){mousedown=true;}else{mousedown=false;}});_this.$outer.on('mousemove.lg',function(){mousedown=false;});_this.$outer.on('mouseup.lg',function(e){if($(e.target).is('.lg-outer')||$(e.target).is('.lg-item ')||$(e.target).is('.lg-img-wrap')&&mousedown){if(!_this.$outer.hasClass('lg-dragging')){_this.destroy();}}});}};Plugin.prototype.destroy=function(d){var _this=this;if(!d){_this.$el.trigger('onBeforeClose.lg');$(window).scrollTop(_this.prevScrollTop);}/**
...
$.each($.fn.lightGallery.modules,function(key){if(_this.modules[key]){_this.modules[key].destroy();}});this.lGalleryOn=false;clearTimeout(_this.hideBarTimeout);this.hideBarTimeout=false;$(window).off('.lg');$('body').removeClass('lg-on lg-from-hash');if(_this.$outer){_this.$outer.removeClass('lg-visible');}$('.lg-backdrop').removeClass('in');setTimeout(function(){if(_this.$outer){_this.$outer.remove();}$('.lg-backdrop').remove();if(!d){_this.$el.trigger('onCloseAfter.lg');}_this.$el.focus();},_this.s.backdropDuration+50);};$.fn.lightGallery=function(options){return this.each(function(){if(!$.data(this,'lightGallery')){$.data(this,'lightGallery',new Plugin(this,options));}else{try{$(this).data('lightGallery').init();}catch(err){console.error('lightGallery has not initiated properly',err);}}});};$.fn.lightGallery.modules={};})();;/* script-start */$('#hamburger').click(function(){$('#navbar-collapse').addClass('show');});$('#navbar-collapse-close').click(function(){$('#navbar-collapse').removeClass('show');});$('#header-search-mobile').click(function(){$('#header-search').toggleClass('hid');});if(window.innerWidth<1280){$('.has-subnav').on('click',function(){$(this).next().toggleClass('show');});}$('.slider').lightSlider({item:1,loop:true,controls:false});$('.partners-slider').lightSlider({item:4,loop:true,pager:false,prevHtml:"\n        <svg xmlns=\"http://www.w3.org/2000/svg\" width=\"7.961\" height=\"14.826\" viewBox=\"0 0 7.961 14.826\" style=\"padding-right: 1px\">\n            <path d=\"M0,0-5.547,6.177,0,12\" transform=\"translate(6.547 1.412)\" fill=\"none\" stroke=\"#fff\" stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\"/>\n        </svg>      \n    ",nextHtml:"\n        <svg xmlns=\"http://www.w3.org/2000/svg\" width=\"7.961\" height=\"14.826\" viewBox=\"0 0 7.961 14.826\" style=\"padding-left: 1px\">\n            <path id=\"\u041A\u043E\u043D\u0442\u0443\u0440_17\" data-name=\"\u041A\u043E\u043D\u0442\u0443\u0440 17\" d=\"M0,0-5.547,6.177,0,12\" transform=\"translate(1.414 13.414) rotate(180)\" fill=\"none\" stroke=\"#fff\" stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\"/>\n        </svg>\n    ",responsive:[{breakpoint:1024,settings:{item:3}},{breakpoint:600,settings:{item:2}},{breakpoint:480,settings:{item:1}}]});$('.information-company-diploma').lightSlider({gallery:false,item:4,loop:true,enableDrag:false,pager:false,prevHtml:"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"7.961\" height=\"14.826\" viewBox=\"0 0 7.961 14.826\" style=\"padding-right: 1px\">\n            <path d=\"M0,0-5.547,6.177,0,12\" transform=\"translate(6.547 1.412)\" fill=\"none\" stroke=\"#fff\" stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\"/>\n        </svg>",nextHtml:"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"7.961\" height=\"14.826\" viewBox=\"0 0 7.961 14.826\" style=\"padding-left: 1px\">\n            <path id=\"\u041A\u043E\u043D\u0442\u0443\u0440_17\" data-name=\"\u041A\u043E\u043D\u0442\u0443\u0440 17\" d=\"M0,0-5.547,6.177,0,12\" transform=\"translate(1.414 13.414) rotate(180)\" fill=\"none\" stroke=\"#fff\" stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\"/>\n        </svg>",onSliderLoad:function onSliderLoad(el){el.lightGallery({selector:'.information-company-diploma .lslide'});},responsive:[{breakpoint:1200,settings:{item:3}},{breakpoint:1000,settings:{item:2}},{breakpoint:460,settings:{item:1}}]});$('.reviews-slider').lightSlider({gallery:false,item:5,loop:true,enableDrag:false,pager:false,prevHtml:"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"7.961\" height=\"14.826\" viewBox=\"0 0 7.961 14.826\" style=\"padding-right: 1px\">\n          <path d=\"M0,0-5.547,6.177,0,12\" transform=\"translate(6.547 1.412)\" fill=\"none\" stroke=\"#fff\" stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\"/>\n      </svg>",nextHtml:"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"7.961\" height=\"14.826\" viewBox=\"0 0 7.961 14.826\" style=\"padding-left: 1px\">\n          <path id=\"\u041A\u043E\u043D\u0442\u0443\u0440_17\" data-name=\"\u041A\u043E\u043D\u0442\u0443\u0440 17\" d=\"M0,0-5.547,6.177,0,12\" transform=\"translate(1.414 13.414) rotate(180)\" fill=\"none\" stroke=\"#fff\" stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\"/>\n      </svg>",onSliderLoad:function onSliderLoad(el){el.lightGallery({selector:'.reviews-slider .lslide'});},responsive:[{breakpoint:1024,settings:{item:3}},{breakpoint:700,settings:{item:2}},{breakpoint:460,settings:{item:1}}]});var imageGallery=$('#imageGallery').lightSlider({gallery:true,item:1,loop:true,thumbItem:4,thumbMargin:50,slideMargin:0,controls:false,enableDrag:true,onSliderLoad:function onSliderLoad(el){el.lightGallery({selector:'#imageGallery .lslide'});}});$('.product-page__slider').prepend("\n  <button id='product-page__slider-prev' class='product-page__slider-prev'>\n    <i class=\"_icon-angle-left\"></i>\n  </button>\n  <button id='product-page__slider-next' class='product-page__slider-next'>\n    <i class=\"_icon-angle-right\"></i>\n  </button>\n");$('#product-page__slider-prev').on('click',function(){imageGallery.goToPrevSlide();});$('#product-page__slider-next').on('click',function(){imageGallery.goToNextSlide();});$('#button-dropdown-contacts').click(function(){$('#block-dropdown-contacts').toggleClass('d-block');});$('.documentation__description-slider').lightSlider({item:1,loop:true,controls:false});$('.button-dropdown-phone').click(function(){$('#block-dropdown-phone').addClass('d-block');if(window.innerWidth<1010){$('body').addClass('modal-open');}});$('#block-dropdown-phone__close').click(function(){$('#block-dropdown-phone').removeClass('d-block');if(window.innerWidth<1010){$('body').removeClass('modal-open');}});$('#block-dropdown-phone .modal-windows__bg').click(function(){$('#block-dropdown-phone').removeClass('d-block');if(window.innerWidth<1010){$('body').removeClass('modal-open');}});});